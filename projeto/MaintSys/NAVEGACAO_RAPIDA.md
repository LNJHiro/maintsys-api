# NAVEGACAO_RAPIDA — MaintSys Cheat Sheet

Guia para localizar qualquer elemento do projeto: views, controllers, models, rotas e banco.
...
| Coluna ID                           | `VW/tecnicos/index.blade.php`    | 54      | `$t->id`                             |
...
| Coluna E-mail                       | `VW/tecnicos/index.blade.php`    | 58      | `$t->email`                          |
...
| Botão Ver                           | `VW/tecnicos/index.blade.php`    | 90      | Sempre visível                       |
...
| Botão Criar O.S.                    | `VW/ordens/create.blade.php`     | 88      | `type="submit"`                      |
...
| Dropdown Tipo                       | `VW/ordens/edit.blade.php`       | 48–65   | Preventiva / Corretiva               |
| Dropdown Prioridade                 | `VW/ordens/edit.blade.php`       | 56–64   | Baixa / Média / Alta / Crítica       |
| Dropdown Status                     | `VW/ordens/edit.blade.php`       | 69–76   | Aberta / Em Andamento / Concluída... |
| Campo Data Prevista                 | `VW/ordens/edit.blade.php`       | 77–81   | `name="data_prevista"`               |
| Campo Descrição                     | `VW/ordens/edit.blade.php`       | 84–87   | Textarea `name="descricao"`          |
...
| Campo Peças Utilizadas              | `VW/ordens/edit.blade.php`       | 114–118 | `name="pecas_utilizadas"`, textarea  |
| Campo Próxima Preventiva (automático)| `VW/ordens/edit.blade.php`      | 121–132 | `name="proxima_preventiva"`, date    |
| Botão Salvar Alterações             | `VW/ordens/edit.blade.php`       | 135     | `type="submit"`                      |
...
| Seção Descrição do Problema         | `VW/ordens/show.blade.php`       | 64–69   | Texto livre                          |
...
| Botão Ver                           | `VW/historico/index.blade.php`   | 117     | Sempre visível                       |
...
| Botões Criar / Cancelar             | `VW/usuarios/create.blade.php`   | 121–134 | Submit + link de volta               |
...
| Campo E-mail                        | `VW/usuarios/edit.blade.php`     | 53–71   | `name="email"`                       |
...
| Botões Salvar / Cancelar            | `VW/usuarios/edit.blade.php`     | 113–128 | Submit + link de volta               |
...
| Header do módulo (Máquinas, Técnicos...) | `VW/usuarios/permissions.blade.php` | 42–58 | Switch com ícones emoji          |
...
| Link "Criar conta"                  | `VW/auth/login.blade.php`        | 52–55   | Exibido se rota `register` existe    |
...
### Registro e Auto-cadastro

| O que quero                          | Arquivo                          | Linhas  | Detalhe                              |
|-------------------------------------|----------------------------------|---------|--------------------------------------|
| Auto-registro desabilitado          | `routes/auth.php`                | 14–16   | Comentado intencionalmente           |
| Criar usuário (pelo admin)          | `VW/usuarios/create.blade.php`   | 1–171   | Via `/usuarios/criar`                |
...
| Tabela                    | Model                   | Linhas model        |
|--------------------------|-------------------------|---------------------|
| `users`                   | `MDL/User.php`          | 1–120               |
| `tecnicos`                | `MDL/Tecnico.php`       | 1–71                |
| `maquinas`                | `MDL/Maquina.php`       | 1–100               |
| `ordens_servico`          | `MDL/OrdemServico.php`  | 1–140               |
| `historico_manutencoes`   | `MDL/HistoricoManutencao.php` | 1–70          |
| `permissions`             | `MDL/Permission.php`    | 1–30                |
| `role_permissions`        | `MDL/RolePermission.php`| 1–25                |
| `user_permissions`        | `MDL/UserPermission.php`| 1–25                |
...
**Como funciona:**
1. Usuário clica no botão "🖨️ Imprimir"
2. JavaScript chama `window.print()` (função nativa do navegador)
3. Abre a caixa de diálogo de impressão do navegador
4. Usuário escolhe: imprimir para papel, PDF, arquivo, etc.

**Não há:**
- CSS especial de print
- Lógica server-side
- Formatação customizada
- Esconde/mostra elementos diferentes

**Usa:**
- CSS padrão do aplicativo (Tailwind)
- Estilo que está na tela é o que será impresso
- O navegador gerencia a quebra de página automaticamente

**Melhorias possíveis:**
```
1. Adicionar @media print {...} em resources/css/app.css
   - Esconder botões de ação (Editar, Deletar, etc.)
   - Esconder paginação
   - Ajustar cores para preto/branco
   - Adicionar quebras de página

2. Adicionar <link media="print" href="..."> no layout.app.blade.php
...
1. CTR/OrdemServicoController.php:367  → sincronizarStatusMaquina()
   Está sendo chamada em:
   - store()   linha 82
   - update()  linha 207
   - destroy() linha 356

2. Lógica da função (linhas 367–407):
...
3. Check banco:
   SELECT * FROM historico_manutencoes ORDER BY id DESC LIMIT 5;
```
...
4. Para grant rápido (debug):
   UPDATE role_permissions SET role='admin' WHERE permission_id=X;
```
...
**Versão:** 2.0 — **Atualizado:** 2026-06-08
